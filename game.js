...
const SKILLDEFS = {
  ...
  bubble:  { name:'Bubble Prison',  emoji:'🫧', max:5, desc:'จับศัตรูไว้ในฟอง ก่อนระเบิดทำดาเมจรอบเป้าหมาย',
    awaken:{ name:'เรือนจำฟองแตก', emoji:'🫧', desc:'ยิงหลายฟอง จับนานขึ้น และระเบิดลูกโซ่วงใหญ่!' } },
  aura:    { name:'Sakura Aura',    emoji:'🌸', max:5, desc:'กลีบซากุระสะสม Bloom; ครบ 3 ชั้นจะแตกเป็นดาเมจวงและสโลว์',
  ...
  mine:    { name:'Cupcake Sentry',   emoji:'🧁', max:5, desc:'วางป้อมคัพเค้กยิงครีมอัตโนมัติ แล้วระเบิดปิดท้าย',
  ...
  meteor:  { name:'Bear-Sigil Donut', emoji:'🍩', max:5, desc:'ตราหมีโกโก้กระแทกซ้ำ ทิ้ง Chocolate Glaze ชะลอศัตรู',
  ...
  mirror:  { name:'Mirror Glaze', emoji:'🪞', max:5, desc:'กางเคลือบกระจกสะท้อนกระสุนศัตรูกลับเป็นกระสุนรสชาติ',
    awaken:{ name:'กระจกพันรส', emoji:'🪞', desc:'วงสะท้อนใหญ่ขึ้นและยิงเศษกระจกไล่เป้าเพิ่ม!' } },
  memory:  { name:'Memory Jam', emoji:'🫙', max:5, desc:'ประทับความทรงจำบนเป้าหมาย แล้วระเบิดซ้ำตามดาเมจที่สะสม',
    awaken:{ name:'แยมแห่งวันวาน', emoji:'🫙', desc:'ประทับหลายเป้าหมายและสะท้อนความเสียหายเป็นวงกว้าง!' } },
  thread:  { name:'Flavor Thread', emoji:'🧵', max:5, desc:'ผูกศัตรูเป็นสายใย แชร์ดาเมจและดึงเข้าหาศูนย์กลาง',
    awaken:{ name:'สายใย Flavorbound', emoji:'🧵', desc:'ผูกศัตรูเพิ่มและกระชากซ้ำแรงขึ้น!' } },
  decoy:   { name:'Core Decoy', emoji:'💠', max:5, desc:'สร้างแก่นรสลวง ดึงฝูงออกจากผู้เล่นก่อนระเบิด',
    awaken:{ name:'แก่นลวงสมบูรณ์', emoji:'💠', desc:'ล่อได้นาน ระเบิดสองชั้น และทิ้งพลังฟื้นฟู!' } },
  triseal: { name:'Triad Seal', emoji:'🔺', max:5, desc:'วางตราสามจุด; จุดที่สามเชื่อมสามเหลี่ยมแล้วชำระล้างพื้นที่',
    awaken:{ name:'พันธสัญญาสามรส', emoji:'🔺', desc:'ตรากว้างขึ้นและฟาดศัตรูตลอดแนวทั้งสามด้าน!' } },
  echoStep:{ name:'Echo Step', emoji:'👣', max:5, desc:'ทิ้งรอยก้าวย้อนหลังตามเส้นทางเคลื่อนที่ ก่อนระเบิดตามลำดับ',
...
const CHARACTER_UNIQUES = {
  ...
  pathRecall:{name:'เส้นทางหวนคืนของตาโร่',emoji:'👣',cd:13,color:0xb388ff,desc:'จุดชนวนรอยเท้าย้อนหลังและรีเซ็ต Dash ทันที'},
  oathMirror:{name:'กระจกคำสัตย์งาดำ',emoji:'🪞',cd:15,color:0xd8d9e2,desc:'ชำระกระสุนศัตรูทั้งหมดและสร้างเกราะอมตะชั่วครู่'},
};
...
const CHAR_TALENTS = {
  ...
  taro: [
    ...
    { id:'echoPath', emoji:'👣', name:'แผนที่เสียงสะท้อน', max:1, per:'✦ Unique: เส้นทางหวนคืนเพิ่มรอยเท้าและรัศมีระเบิด!', apply:(p,r)=>{ p.echoPath=true; } },
  ],
  sesame: [
    ...
    { id:'mirrorWard', emoji:'🪞', name:'กระจกคำสัตย์', max:1, per:'✦ Unique: เกราะคำสัตย์คงอยู่นานขึ้น!', apply:(p,r)=>{ p.mirrorWard=true; } },
  ],
};
...
class Game extends Phaser.Scene {
  ...
  useCharacterSkill(){
    ...
    if(c.unique==='berryRebound'){
      ...
      this.castEchoStep(this.player.echoPath?5:4,true,dm);this.dashReady=true;this.dashCd=0;Sfx.dash();
    }else{
      let reflected=0;this.foeBullets.children.iterate(f=>{if(!f||!f.active)return;const x=f.x,y=f.y;this.killFoe(f);const b=this.getBullet(x,y,0xd8d9e2,0.30);if(!b)return;b.dmg=20*dm;b.life=2;b.homing=500;b.pierce=true;b.faceVel=true;this.physics.velocityFromRotation(Math.random()*Math.PI*2,360,b.body.velocity);reflected++;});
      this.player.iframe=Math.max(this.player.iframe,this.player.mirrorWard?2.2:1.6);this.showBanner('🪞 กระจกคำสัตย์','ชำระกระสุน '+reflected+' นัด · เกราะคำสัตย์ทำงาน',950);Sfx.zap();
    }
  }
  ...
  buildSkillArchive(){
    ...
    keys.slice(page*perPage,page*perPage+perPage).forEach((key,i)=>{const d=defs[key],col=i%cols,row=Math.floor(i/cols),x=side+col*(cardW+gap),y=top+row*(cardH+gap),color=isPass?(d.color||COLORS.mint):0xf0a54a;
      ...
      this.menu.add([g,ic,nm,desc,ft]);this._zone(x,y,cardW,cardH,()=>{this._skillArchiveSelected={key,isPass};this.buildSkillArchive();});
    });
  ...
  buildSkillArchiveDetail(sel,portrait,top){
    ...
    const type=this.add.text(tx,panelY+43,sel.isPass?'PASSIVE · สูงสุด 5 ดาว':'ATTACK · สูงสุด 5 ดาว',{fontFamily:'sans-serif',fontStyle:'bold',fontSize:'9px',color:sel.isPass?'#8fe6c5':'#ffd08a'}).setOrigin(0,0);
  ...
  buildChars(){
    ...
    const rowH=landscape?Math.min(76,(this.H-108)/3):80, gap=landscape?7:12, totalW=cardW*cols+(cols-1)*10;
    const x0=(this.W-totalW)/2, y0=landscape?91:this.H*0.15;
    CHAR_ORDER.forEach((id,i)=>{ const c=CHARACTERS[id], col=i%cols, row=Math.floor(i/cols), y=y0+row*(rowH+gap), x=x0+col*(cardW+10);
      ...
      this._rowBtn(y,rowH,c.emoji,c.name,'Unique: '+CHARACTER_UNIQUES[c.unique].name+' · '+c.desc,
  ...
  buildGearLandscape(){
    ...
    if(this.textures.exists('char_'+id)){const sp=this.add.image(pcx,pcy,'char_'+id,0).setScale(0.92);this.menu.add(sp);}else{const em=this.add.text(pcx,pcy,CHARACTERS[id].emoji,{fontSize:'54px'}).setOrigin(0.5);this.menu.add(em);}
  ...
  waveProfile(w){
    ...
    if(si===1){const drain=[
      {name:'ฟองหลุดท่อ',desc:'ฟองเน่าลอยช้าและพ่นน้ำเสียจากระยะกลาง',dur:46,interval:1.50,batch:2,max:26,types:['basic','basic','shooter']},
      {name:'กระแสไหลย้อน',desc:'ตัวโฉบตามกระแสน้ำบังคับให้เปลี่ยนทางเดิน',dur:48,interval:1.12,batch:3,max:36,types:['fast','dasher','basic']},
      {name:'วาล์วเดือด',desc:'ฟองเดือดผู้เฝ้าวาล์วใช้วงล้อมและกระสุนฟอง',dur:0,interval:1.50,batch:2,max:30,types:['basic','shooter','fast']},
      {name:'ตะแกรงอุดตัน',desc:'ฟองเกราะหนาบีบพื้นที่ ขณะฟองพ่นยิงจากด้านหลัง',dur:50,interval:1.32,batch:2,max:34,types:['tank','basic','shooter','bomber']},
    ...
    const profiles=[
      ...
      {name:'บททดสอบสุดท้าย',desc:'เวฟผสมก่อนเผชิญหน้าบอส',dur:56,interval:0.92,batch:4,max:52,types:si===0?['acid','dasher','tank','basic']:['fast','dasher','shooter','bomber','tank','siege']}
    ];
  ...
  getBullet(x,y,tint,scale){
    ...
    b.bounce=0; b.rebound=false; b.reb=0; b.spin=false; b.homing=0; b.explode=0; b.faceVel=false; b.chain=0; b.bubblePrison=false; b.bubbleAwaken=false;
    return b;
  }
  ...
  castSkill(key,lvl){
    ...
    if(key==='sprinkle'){ const t=this.nearestEnemy(aw?900:640); if(!t)return;
      ...
      for(let i=0;i<cnt;i++){ const ang=Math.random()*Math.PI*2;
        const b=this.getBullet(this.player.x,this.player.y,0xffffff,0.27+lvl*0.018+(aw?0.07:0)); b.setTexture('proj_popcorn').setTint(0xffffff); b.faceVel=true; b.dmg=dmg; b.life=aw?1.8:1.4; b.pierce=false; b.bounce=bounce; b.spin=true; b.hitGapV=0.12;
      ...
      for(let s=0;s<cnt;s++){ const ang=Math.random()*Math.PI*2;
        ...
        b.bubblePrison=true; b.bubbleAwaken=aw; b.bubbleLevel=lvl; b.bubbleRadius=(62+lvl*9)*(aw?1.3:1);
  ...
  hitEnemy(bullet,enemy){ if(!bullet.active||!enemy.active)return;
    if(bullet.bubblePrison){
      const x=enemy.x,y=enemy.y,lvl=bullet.bubbleLevel||1,r=bullet.bubbleRadius||70,aw=!!bullet.bubbleAwaken,dmg=bullet.dmg||8;
      ...
      if(enemy.active){ enemy.frozen=Math.max(enemy.frozen||0,enemy.isBoss?0.28:(0.62+lvl*0.10));
        ...
        this.time.delayedCall(430+lvl*35,()=>{ const bx=enemy.active?enemy.x:x,by=enemy.active?enemy.y:y; cage.destroy();
          ...
          this.enemies.children.iterate(e=>{ if(e&&e.active&&this.dist(e.x,e.y,bx,by)<r)this.damage(e,dmg*(aw?1.25:0.90),e.x,e.y); });
  ...
  update(time,delta){
    ...
    this.moveSlowT=Math.max(0,(this.moveSlowT||0)-dt);
    ...
    if(this.dashTime>0){ this.dashTime-=dt; if(this.dashTime<=0){ this._sqX=0.8; this._sqY=1.22; this._sqVX=0; this._sqVY=0; this.poseFlash(CF.squash,150); } }
      ...
      const spd=this.player.baseSpeed*(this.moveSlowT>0?0.52:1);
