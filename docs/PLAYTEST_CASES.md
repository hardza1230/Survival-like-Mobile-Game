...
| BAL-10 | Power Rating ต้องเทียบข้าม build ได้ | ตรวจสูตรจากเลเวลตัวละคร/ยศ/พร/อุปกรณ์/Mastery และเทียบอัตราชนะกับค่าพลังแนะนำ 100/280/560/940/1450 |
| BAL-11 | Power Guide ต้องลดหน้าผาความยากโดยไม่ลบ progression | พลังต่ำสุดช่วย HP/ดาเมจศัตรูไม่เกินประมาณ 19%/15% และเพิ่ม catch-up reward สูงสุด 30%; พลังสูงกว่าคำแนะนำต้องไม่ทำให้ศัตรูสเกลตาม |
| BAL-12 | ต่อหนึ่งด่านมี Awaken ไม่เกิน 2 | เมื่อถือ Attack MAX + Passive ครบคู่ที่สาม การ์ด Awaken ต้องไม่เข้ากองอีก; รีเซ็ตเพดานเมื่อเริ่มด่านใหม่ |
| BAL-13 | โค้ง EXP และฐานผู้เล่นต้องผลักให้พัฒนา meta | EXP เริ่ม 7 และโต `×1.20+3`; ผู้เล่นฐาน HP 90, damage ×0.90, speed 166, crit ×1.55 แต่ Power Guide ยังช่วยผู้เล่นต่ำกว่าคำแนะนำตามเพดานเดิม |
...
| SKL-11 | Passive card ต้องบอกว่าจับคู่กับอะไรดี | Passive ที่มี Awaken แสดงสกิลโจมตีคู่ครบทุกคู่; ใบที่ไม่มีคู่ตรงแสดงสาย build ที่เหมาะ โดยข้อความไม่ทับดาว/ปุ่มยืนยัน |
| SKL-12 | Unique ทุกเลเวลต้องอลังการขึ้น ไม่ใช่แค่แรงขึ้น | เทียบ Lv1–4 ครบ 5 ตัว: จำนวนวงแหวน/ประกาย, flash, shake และ finisher เพิ่มทีละขั้นโดยไม่บดบัง telegraph อันตราย |
...
| CHR-05 | Taro Unique ต้องตอบแทนการวางเส้นทาง | เดินเป็นเส้นยาว/สั้นแล้วเทียบจำนวนจุด ลำดับระเบิด ตัวคูณมีเพดาน การรีเซ็ต Dash และ path haste |
| CHR-06 | Sesame Unique ต้องเป็นเขตป้องกันเชิงตำแหน่ง | กระสุนนอกเขตไม่ถูกลบ กระสุนเข้าเขตถูกสะท้อน จำนวนประจุสัมพันธ์กับ pulse/final blast และ VFX ถูก cleanup |
| CHR-07 | หน้าเลือกตัวและอุปกรณ์ใช้ Character Card | ทั้ง 5 ตัวใช้ภาพ 768×1024 alpha จริง ไม่แสดงเฟรมจาก sprite sheet; ตรวจสถานะล็อก/เลือกและ fallback เมื่อ texture หาย |
...
| VIS-07 | Clogmaw flipbook 8 เฟรมต้องตรงกับท่า | ตรวจ 4×2 ไม่มี bleed ระหว่างเซลล์, anchor ไม่กระโดด, reveal เล่นเฟรม 2–3, prison/suction/overflow ใช้ 4/5/6 และ phase ใช้เฟรม 7 ก่อนกลับ idle 0–1 |
| VIS-08 | สี่เหลี่ยมดำ 352×366 ระหว่างบอสด่าน 2 | VFX จาก `ASSET_FX` ต้องใช้ ADD เป็นค่าเริ่มต้น รวม aura/enrage; ทดสอบ suction/overflow และ Unique ทุกตัวบน Android/WebGL โดยต้องเห็นฉากหลังผ่านพื้นที่ดำของชีต |
...
## I. Character Profiles & Signature Weapons (v2.29.0)

| ID | เคส | ผลที่ต้องได้ |
|---|---|---|
| CHR-01 | เริ่มรันทั้ง 5 ตัวละคร | มีอาวุธประจำตัว Lv1 ก่อนเลือกอาวุธรอง |
| CHR-02 | หน้าเลือกอาวุธ | แสดง Mastery และสุ่มอาวุธรอง 3 ใบไม่ซ้ำอาวุธประจำตัว |
| CHR-03 | เลือกอาวุธรอง | ถืออาวุธ 2 ชิ้นโดยอาวุธประจำตัวไม่หาย |
| CHR-04 | โมโม่ใช้ Sprinkle | ยิงเพิ่ม 2 นัด คูลดาวน์ -10% |
| CHR-05 | มินต์ใช้ Frost | รัศมี +14% แช่นาน +18% คูลดาวน์ -14% |
| CHR-06 | โกโก้ใช้ Meteor | ดาเมจ/พื้นที่เพิ่ม คูลดาวน์ +5% |
| CHR-07 | ตาโร่ใช้ Thunder | ชิ่งเพิ่ม 2 เป้า คูลดาวน์ -14% |
| CHR-08 | งาดำใช้ Mirror | เขตกว้าง +12% สะท้อนเพิ่ม 2 นัด คูลดาวน์ -12% |
| CHR-09 | ผ่านประตูด่านถัดไป | คืนอาวุธประจำตัว Lv1 ไม่เริ่มมือเปล่า |
| CHR-10 | การ์ดตัวละครแนวตั้ง/แนวนอน | ชื่อ อาวุธ บทบาท และเรตไม่ทับปุ่ม |
| CHR-11 | อาวุธครบ 4 ช่อง | อาวุธประจำตัวนับรวมและไม่เกิน SKILL_CAP |
| CHR-12 | Awaken อาวุธประจำตัว | Mastery ยังทำงานและไม่บัฟอาวุธรอง |
