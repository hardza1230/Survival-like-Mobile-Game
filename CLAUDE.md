...
- `assets/` — รูปจริง (AI/วาดมือ) โหลดแทนกราฟิกโค้ด · `ASSET_IMAGES`=รูปนิ่ง · `ASSET_SHEETS`=สไปรต์สตริปหลายเฟรม (game.js)
  ...
  · **v1.6.1:** ปุ่มลูกกวาด `ui_btn_pink/mint`(420×138 อัตราส่วน ~3.04:1) helper `uiPillBtn(cont,cx,cy,w,h,tex,label,textColor,fn)` (image+ข้อความเลื่อนขวาพ้นสตรอว์เบอร์รี+stroke ขาว) เสียบใน buildHub(4 ปุ่มสลับสี) + buildPause(2 ปุ่ม) · fallback graphics ถ้ารูปโหลดไม่ได้
